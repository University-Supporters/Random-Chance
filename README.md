...
- `SESSION_SECRET`에 32자 이상의 무작위 비밀키를 설정하면 모든 서버 인스턴스가 동일한 키로 관리자 토큰을 검증합니다. 모든 인스턴스에 같은 값을 사용하세요.
- 별도 설정이 없으면 기존 `KV_REST_API_TOKEN` 또는 `GITHUB_TOKEN`으로 용도가 분리된 인증키를 파생합니다. 해당 자격 증명을 교체하면 기존 로그인 세션도 종료됩니다.
- 원격 자격 증명이 없는 로컬/Docker 실행에서는 `data/.session-secret`을 자동 생성하여 재시작 후에도 재사용합니다. 이 파일은 DB 다운로드·복원 대상에 포함되지 않습니다.
- Vercel에서 위 공유 비밀키가 모두 없으면 설정 안내를 반환합니다. 인스턴스마다 다른 임시 키를 발급하지 않습니다.
- 일반 관리자와 상급 관리자 권한, 상급 토큰의 로그인 세션 연결 및 만료 시간은 그대로 유지합니다.
